...
train_images = np.loadtxt('E:\Facultate\IA\data\data\\train_images.txt') # incarcam imaginile
train_labels = np.loadtxt('E:\Facultate\IA\data\data\\train_labels.txt', 'int') # incarcam etichetele avand tipul de date int
...
class NaiveBayes:
    def __init__(self, num_bins, max_value):
        ...
        self.bins = np.linspace(0, max_value, num=num_bins) # returneaza intervalele
        ...
        for pos in range(self.num_features):
            for idx_bin in range(self.num_bins):
                for class_id in range(self.num_classes):
                    ...
                    position_bin_class_prob[pos][idx_bin][class_id] = np.sum(
                        train_images_class[:, pos] == idx_bin) / train_images_class[:, pos].shape[0]
    ...
    def predict(self, test_images):
        ...
        for class_id in range(self.num_classes):
            for idx_img in range(num_images):
                ...
                for pos in range(self.num_features):
                    probs[idx_img][class_id] += np.log(
                        self.position_bin_class_prob[pos][int(test_images[idx_img][pos])][class_id])
...
train_labels = np.loadtxt('data/data/train_labels.txt', 'int')
test_images = np.loadtxt('data/data/test_images.txt')
test_labels = np.loadtxt('data/data/test_labels.txt', 'int')
